...
function save_playlists(){
	...
	for (var i = 0; i < array_length(global.playlists); i++) {
		...
		for (var j = 0; j < ds_list_size(pl.musicas); j++) {
			var musica = ds_list_find_value(pl.musicas, j);

			var cap_index_list = ds_list_create();
			ds_list_add(cap_index_list, ds_list_find_value(musica, 0));
			ds_list_add(cap_index_list, ds_list_find_value(musica, 1));
	...
	var json_text = json_encode(json_list);
