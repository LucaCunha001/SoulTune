	...
	for (var i = 0; i < len_playlist; i++) {
		...
		if (array_length(offset_values) > 0) {
			for (var j = 0; j < array_length(offset_values); j++) {
				...
				else if (val > limiar && ds_map_exists(passados_map, limiar)) {
					offset_y += 25;
				}
			}
		}
		...
		draw_bordered_rect(x1 - 4, btn_top - 4, x1 + btn_width + 4, btn_bottom + 4, 2, (i == hovered_btn[1] && k == hovered_btn[0]));
		draw_set_color(c_white);
		draw_text_customizado(x1 + 10, y1 + 10 + offset_y, scr_getmusicname(val, k), 24);
