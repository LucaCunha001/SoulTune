...
for (var i = 0; i<array_length(global.backgrounds); i++) {
	array_push(backgrounds_names, scr_gettext("obj_config_background" + string(i+1)));
}

settings_buttons = [
	scr_gettext("obj_config_idioma") + ": " + scr_gettext("idioma"),
	scr_gettext("obj_config_background") + ": " + backgrounds_names[global.background_index],
...
total_h = (array_length(settings_buttons)+1) * (btn_height + btn_spacing);
base_y = (room_height - total_h) / 2;

slider_w = 180;
slider_h = 10;
slider_x = (room_width - slider_w)/2;
slider_y = base_y + total_h - btn_height - 5;
slider_handle_r = 10;
