...
else if (credit_index < array_length(credits))
{
	...
	for (var i = 0; i < array_length(credits[credit_index]); i++)
	{
		...
		if (_credit.columns == 1) {
			...
			for (var k = 0; k < (array_length(_credit.text_line) / 2); k++)
				draw_text(252, _yy + y_pos + _text_y + (line_height * k) + _y_offset, string_hash_to_newline(_credit.text_line[k]));
			...
			{
				draw_text(228, _yy + y_pos + _text_y + (line_height * line_index) + _y_offset, string_hash_to_newline(_credit.text_line[k]));
				line_index++;
				k++;
			}
